...
export default function TaskswithToDoCorrect() {
  ...
  const fetchActivities = async () => {
    setIsLoading(true);
    try {
      const response = await api.get("/protask/api/tasklist/"); // Replace with your API endpoint to fetch activities
...
function ActivityTasks({ activity }) {
  ...
  const fetchTasks = async () => {
    setIsLoading(true);
    try {
      const response = await api.get(`/protask/api/tasklist/?activity=${activity.id}`); // Fetch tasks for the specific activity
  ...
  const addTodo = async () => {
    if (inputValue.trim() !== "") {
      try {
        const response = await api.post("/protask/api/tasklist/", {
