...
function MyTimeTable() {
  ...
  const handleEventSave = async () => {
    try {
      if (newEvent.id) {
        const response = await api.put(`/protask/api/events/${newEvent.id}/`, newEvent);
        ...
        await api.post("/protask/api/events/", newEvent);
      }
      fetchEvents();
      setDialogOpen(false);
    } catch (error) {
  ...
  const handleEventDelete = async () => {
    try {
      if (newEvent.id) {
        await api.delete(`/protask/api/events/${newEvent.id}/`);
        fetchEvents();
        setDialogOpen(false);
      }
    } catch (error) {
  ...
  const handleEventDrop = async (eventDropInfo) => {
    ...
    try {
      const response = await api.patch(
        `/protask/api/events/${eventDropInfo.event.id}/`,
        updatedEvent
      );
